...
# %% Data Preparation
# 1st example: Raymond Chandler "The Big Sleep" (crime novel)
# 2nd example: J.R.R. Tolkien "The Lord of the Rings" (fantasy novel)
# 3rd example: Bill Bryson "A Short History of Nearly Everything"
documents = [
...
#%% classify documents
res = classifier(documents, candidate_labels)

#%% visualize the possibilities for each category pertaining the 2nd book:
pd.DataFrame(res[1]).plot.bar(x='labels', y='scores', title='Lord of the Rings')

# %% flag multiple labels:
classifier(documents[0], candidate_labels = candidate_labels, multi_label=True)
